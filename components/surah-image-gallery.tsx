...
export function SurahImageGallery({ images, title = "Surah Rahman Pages" }: SurahImageGalleryProps) {
  ...
  const nextPage = () => setCurrentPage((p) => (p + 1) % images.length);
  const prevPage = () => setCurrentPage((p) => (p - 1 + images.length) % images.length);
  const goToPage = (i: number) => setCurrentPage(i);
  ...
  return (
    <div className="space-y-6">
            ...
            <Dialog>
              <DialogTrigger asChild>
                <div className="cursor-pointer group relative">
                  <Image
                    ...
                    alt={currentImage?.alt || "Page"}
                    // Set a sensible intrinsic size you expect in the card
                    width={1200}
                    height={1800}
                    // Helps LCP on first visible image
              ...
              <DialogContent className="max-w-4xl max-h-[90vh] p-2">
                <div className="relative">
                  <Image
                    src={currentImage?.src || "/placeholder.svg"}
                    alt={currentImage?.alt || "Page (zoom)"}
                    width={1600}
                    height={2400}
                    sizes="100vw"
      ...
      {/* Download All Button (kept as is) */}
      <div className="text-center">
